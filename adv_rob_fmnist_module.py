...
def check_sample(sample:tuple[int, TImage, int], base_solve, weights_list, cnfg):
...
def run_test(cfg:CFG):
    ...
    if cfg.z3:
        ...
        # updated: bind the extra arguments with starmap or functools.partial
                    args = [
                        (sample, S, weights_list, cfg)
                        for sample in zip(samples_no_list, sampled_imgs, orig_preds)
                    ]
                    pool.starmap(check_sample, args)
            else:
