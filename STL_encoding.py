...
class Convert_to_Marabou:
    ...
    def encode_marabou(self, spike_times_map):
        ...
        # plus a little headroom for ReLU intermediates, sums, etc.
        headroom = num_spike
        ...
        #─── 2) hand out indices yourself, not via getNewVariable() ───
        next_var = 0
        def new_var():
            nonlocal next_var
            v = next_var
        ...
        #─── 3) allocate spike-time vars up front ─────────────────────
        ...
        # Verification parameters / Requirements
        num_steps = self.num_steps

        ## ensure all spike times  0 <= t <= num_steps - 1
        for layer in range(len(n_layer_neurons)):  # DOUBLE Check if this is LEN IN Z3


            for neuron in (tqdm(enc.get_layer_neurons_iter(layer))):
                s = spike_times_map[(neuron, layer)]
                network.setLowerBound(s, float(layer))
