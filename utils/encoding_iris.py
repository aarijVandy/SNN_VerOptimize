...
def ttfs(vector:List[Union[BoolRef, bool]], ttfs_index:ArithRef) -> List[Union[BoolRef, bool]]:
    n = len(vector)
    eqns:List[Union[BoolRef, bool]] = [typecast(BoolRef, Or([i==ttfs_index for i in range(n)]))]
    for i in range(n):
        eqns.append(
            (i==ttfs_index) == And(
                vector[i],
